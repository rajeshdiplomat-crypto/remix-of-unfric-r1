...
import {
  ArrowLeft, Play, Pause, RotateCcw, Check, Plus, Settings2, Volume2, Music, 
  Sparkles, X, Maximize, Eye, EyeOff, Clock, Quote, Palette, CloudRain,
  Waves, Flame, Trees, Coffee, Wind, Moon, Keyboard
} from "lucide-react";
...
export default function TaskFocus() {
  ...
  const handleSaveSession = async () => {
    ...
    toast({ title: "Session saved!", description: `Added ${sessionMinutes} min of focus time.` });
    navigate("/tasks");
  };
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-black">
        <div className="animate-pulse text-white/60">Loading focus mode...</div>
      </div>
    );
  }
  ...
  if (showSummary) {
    return (
      <div className="fixed inset-0 flex items-center justify-center p-4 z-50">
        <div className="absolute inset-0" style={{ background: getBackground(), backgroundSize: 'cover', backgroundPosition: 'center' }} />
        ...
        <div className="relative z-10 w-full max-w-md bg-black/40 backdrop-blur-2xl rounded-3xl border border-white/10 p-8 space-y-6">
          ...
          <div className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/10 cursor-pointer" onClick={() => setMarkComplete(!markComplete)}>
          ...
          <div className="flex gap-3">
            <Button variant="ghost" className="flex-1 bg-white/5 border border-white/10 text-white hover:bg-white/10" onClick={() => navigate('/tasks')}>Skip</Button>
            <Button className="flex-1 bg-primary hover:bg-primary/90" onClick={handleSaveSession}>Save Session</Button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 overflow-hidden">
      {/* Background */}
      <div className="absolute inset-0 transition-all duration-1000" style={{ background: getBackground(), backgroundSize: 'cover', backgroundPosition: 'center' }} />
      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-black/40" />

      {/* Top Bar */}
      <div className="absolute top-0 left-0 right-0 z-30">
        <div className="flex items-center justify-between px-6 py-4">
          <Button variant="ghost" size="icon" onClick={() => navigate('/tasks')} className="text-white/70 hover:text-white hover:bg-white/10 rounded-xl">
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={toggleFullscreen} className="text-white/70 hover:text-white hover:bg-white/10 rounded-xl">
              <Maximize className="h-5 w-5" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setSettings(s => ({ ...s, zenMode: !s.zenMode }))} className="text-white/70 hover:text-white hover:bg-white/10 rounded-xl">
              {settings.zenMode ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setShowSettings(!showSettings)} className={cn("text-white/70 hover:text-white hover:bg-white/10 rounded-xl", showSettings && "bg-white/10")}>
      ...
      {settings.showQuotes && !settings.zenMode && (
        <div className={cn("absolute top-6 right-24 max-w-xs text-right transition-all duration-700 z-20", quoteVisible ? "opacity-100" : "opacity-0")}>
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="flex gap-2 p-1 bg-black/30 backdrop-blur-xl rounded-full border border-white/10">
              {TIMER_MODES.map(mode => (
                <Button key={mode} variant="ghost" size="sm" onClick={() => {
                  setTimerMode(mode);
                  if (mode === 'Pomodoro') setTimerMinutes(25);
                  if (mode === 'Stopwatch') { stopwatchRef.current = 0; setSecondsRemaining(0); }
                }} className={cn("text-sm px-5 py-2 rounded-full", timerMode === mode ? "bg-white/15 text-white" : "text-white/50 hover:text-white")}>
                  {mode}
                </Button>
              ))}
            </div>
            ...
            <div className="flex gap-3">
              {!isRunning ? (
                <Button size="lg" onClick={() => setIsRunning(true)} className="bg-white text-black hover:bg-white/90 px-12 py-6 text-lg rounded-2xl shadow-lg">
                  <Play className="h-6 w-6 mr-2" /> Start Focus
                </Button>
              ) : (
                <>
                  <Button size="lg" onClick={() => setIsRunning(false)} className="bg-white/10 hover:bg-white/15 text-white px-8 py-6 rounded-2xl border border-white/10">
                    <Pause className="h-5 w-5 mr-2" /> Pause
                  </Button>
                  <Button size="lg" onClick={handleEndSession} className="bg-white/10 hover:bg-white/15 text-white px-8 py-6 rounded-2xl border border-white/10">
                    End
                  </Button>
                </>
              )}
      ...
      <div className="absolute bottom-6 left-6 right-6 z-30">
        <div className="flex items-center justify-between">
          <div className="flex gap-2 p-2 bg-black/30 backdrop-blur-xl rounded-2xl border border-white/10">
            <Button variant="ghost" size="icon" className="text-white/50 hover:text-white hover:bg-white/10 rounded-xl h-10 w-10"><Music className="h-4 w-4" /></Button>
            <Button variant="ghost" size="icon" className="text-white/50 hover:text-white hover:bg-white/10 rounded-xl h-10 w-10"><Volume2 className="h-4 w-4" /></Button>
          </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <Tabs value={settingsTab} onValueChange={setSettingsTab}>
                <TabsList className="w-full grid grid-cols-4 bg-white/5 rounded-xl p-1">
                  <TabsTrigger value="themes" className="rounded-lg text-xs"><Palette className="h-4 w-4" /></TabsTrigger>
                  <TabsTrigger value="clock" className="rounded-lg text-xs"><Clock className="h-4 w-4" /></TabsTrigger>
                  <TabsTrigger value="sounds" className="rounded-lg text-xs"><Volume2 className="h-4 w-4" /></TabsTrigger>
                  <TabsTrigger value="quotes" className="rounded-lg text-xs"><Quote className="h-4 w-4" /></TabsTrigger>
                </TabsList>
