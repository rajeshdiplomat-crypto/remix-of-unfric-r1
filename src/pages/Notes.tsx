...
function StatCard({
  ...
  return <div className="rounded-xl border border-border/40 bg-card shadow-sm hover:shadow-md transition-shadow duration-200">
...
export default function Notes() {
  ...
  const filteredNotes = useMemo(() => {
    return notes.filter(note => {
      ...
      const matchesSearch = q ? note.title.toLowerCase().includes(q) || note.plainText.toLowerCase().includes(q) || note.tags.some(tag => tag.toLowerCase().includes(q)) : true;
  ...
  const syncNoteToSupabase = async (note: Note) => {
    if (!user?.id) return;
    const category: "thoughts" | "creative" | "private" = note.groupId === "personal" ? "private" : note.groupId === "hobby" ? "creative" : "thoughts";
    try {
      const {
        error
  ...
  if (viewMode === "overview") {
    return <div className="w-full flex-1 pb-24">
        {/* Hero */}
        <PageHero storageKey="notes_hero_src" typeKey="notes_hero_type" badge={PAGE_HERO_TEXT.notes.badge} title={PAGE_HERO_TEXT.notes.title} subtitle={PAGE_HERO_TEXT.notes.subtitle} />
        ...
        <div className="w-full space-y-6 px-6 lg:px-8 pt-6">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-end">
            ...
            <div className="flex items-center gap-2 lg:justify-end">
              ...
              <Button variant="outline" size="icon" onClick={() => setSettingsOpen(true)} aria-label="Notes settings" title="Settings" className="h-8 w-10 rounded-xl">
          ...
          <div className="rounded-2xl border border-border/50 bg-card shadow-sm">
            <div className="p-4 flex flex-col gap-3 xl:flex-row xl:items-center">
              ...
              <div className="relative flex-1">
                ...
                <Input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search notes, tags, text..." className="h-10 rounded-xl pl-10 bg-background" />
              </div>
            ...
            <div className="px-4 pb-4">
              <div className="flex gap-2 overflow-auto no-scrollbar">
                <Button variant="outline" size="sm" onClick={() => setFilterGroupId("all")} className={`h-9 rounded-full px-4 ${filterGroupId === "all" ? "bg-primary/10 text-primary border-primary/30" : "text-foreground/80"}`}>
                  All
                </Button>

                {sortedGroups.map(group => {
                const active = filterGroupId === group.id;
                return <Button key={group.id} variant="outline" size="sm" onClick={() => setFilterGroupId(group.id)} className={`h-9 rounded-full px-4 ${active ? "bg-primary/10 text-primary border-primary/30" : "text-foreground/80"}`}>
                      <span className="h-2.5 w-2.5 rounded-full mr-2" style={{
          ...
          {/* Pinned */}
          {pinnedNotes.length > 0 && <div className="rounded-2xl border border-border/50 bg-card shadow-sm">
              ...
              <div className="px-4 pb-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {pinnedNotes.map(n => <button key={n.id} onClick={() => handleNoteClick(n)} className="text-left rounded-2xl border border-border/40 bg-background/60 hover:bg-background/80 hover:shadow-sm transition-all p-4">
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <span>{formatDistanceToNow(new Date(n.updatedAt), {
                    addSuffix: true
                  })}</span>
                    </div>
                  </button>)}
          ...
          {/* Content */}
          {notesView === "atlas" && <div className="space-y-4">
              {sortedGroups.filter(g => filterGroupId === "all" || g.id === filterGroupId).map(group => <NotesGroupSection key={group.id} group={group} folders={folders} notes={filteredNotes} selectedNoteId={selectedNote?.id} onNoteClick={handleNoteClick} onAddNote={handleCreateNote} onAddFolder={handleCreateFolder} />)}
            </div>}

          {notesView === "board" && <NotesBoardView groups={sortedGroups.filter(g => filterGroupId === "all" || g.id === filterGroupId)} folders={folders} notes={filteredNotes} selectedNoteId={selectedNote?.id} onNoteClick={handleNoteClick} onAddNote={handleCreateNote} onAddFolder={handleCreateFolder} />}

          {notesView === "mindmap" && <NotesMindMapView groups={sortedGroups.filter(g => filterGroupId === "all" || g.id === filterGroupId)} folders={folders} notes={filteredNotes} selectedNoteId={selectedNote?.id} onNoteClick={handleNoteClick} onAddNote={handleCreateNote} onAddFolder={handleCreateFolder} />}

          {/* Empty */}
          {sortedGroups.length === 0 && <div className="rounded-2xl border border-border/50 bg-card shadow-sm p-10 text-center">
          ...
          {/* Settings Dialog */}
          <NotesGroupSettings open={settingsOpen} onOpenChange={setSettingsOpen} groups={groups} onGroupsChange={handleGroupsChange} folders={folders} onFoldersChange={setFolders} />

          {/* Location Picker */}
          <NotesLocationPicker open={locationPickerOpen} onOpenChange={setLocationPickerOpen} groups={groups} folders={folders} onConfirm={handleCreateNote} />
        </div>
      </div>;
  }
  ...
  // EDITOR (Split View)
  // =========================
  return <NotesSplitView notes={filteredNotes} groups={groups} folders={folders} selectedNote={selectedNote} onSelectNote={setSelectedNote} onSaveNote={handleSaveNote} onDeleteNote={handleDeleteNote} onBack={handleBackToOverview} onCreateNote={handleQuickNote} />;
}