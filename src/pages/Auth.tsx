...
const ModuleSection = ({
  ...
  imageSrc,
  imageLeft
}: ModuleSectionProps) => <section className="min-h-screen flex flex-col lg:flex-row">
    {imageLeft ? <>
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 lg:px-16 xl:px-24 py-16">
          ...
          <ul className="space-y-3">
            {features.map((feature, i) => <li key={i} className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground">
                — {feature}
              </li>)}
          </ul>
        </div>
      ...
      <section className="min-h-screen flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 md:px-16 xl:px-24 py-12 lg:py-0">
          <div className="max-w-sm mx-auto w-full">
            {/* Brand name */}
            <UnfricLogo size="xl" className="mb-16 my-[10px]" />
            ...
            {mode === "verify-email" ? <div className="space-y-8">
                ...
                <Button onClick={handleResendVerification} variant="outline" className="w-full h-12 uppercase tracking-[0.15em] text-xs font-normal border-foreground hover:bg-foreground hover:text-background transition-all" disabled={isSubmitting}>
                  {isSubmitting ? "SENDING..." : "RESEND VERIFICATION EMAIL"}
                </Button>
                <button type="button" onClick={() => setMode("signin")} className="flex items-center gap-2 text-[11px] uppercase tracking-[0.1em] text-muted-foreground hover:text-foreground transition-colors">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  {/* Email field */}
                  <div>
                    <label htmlFor="email" className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground block mb-3">
                      Email
                    </label>
                    <input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} disabled={isSubmitting} className="w-full bg-transparent border-0 border-b border-border focus:border-foreground outline-none py-3 text-sm text-foreground transition-colors placeholder:text-muted-foreground/50" placeholder="" />
                  </div>

                  {/* Password field */}
                  {mode !== "forgot-password" && <div>
                      <div className="flex items-center justify-between mb-3">
                        <label htmlFor="password" className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">
                          Password
                        </label>
                        {mode === "signin" && <button type="button" onClick={() => setMode("forgot-password")} className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground hover:text-foreground transition-colors">
                            Forgot password?
                          </button>}
                      </div>
                      <input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} disabled={isSubmitting} className="w-full bg-transparent border-0 border-b border-border focus:border-foreground outline-none py-3 text-sm text-foreground transition-colors" />
                    </div>}

                  {/* Submit button */}
                  <div className="pt-4">
                    <Button type="submit" variant="outline" className="w-full h-12 uppercase tracking-[0.15em] text-xs font-normal border-foreground hover:bg-foreground hover:text-background transition-all" disabled={isSubmitting}>
                      {isSubmitting ? "PLEASE WAIT..." : mode === "forgot-password" ? "SEND RESET LINK" : mode === "signup" ? "CREATE ACCOUNT" : "LOG IN"}
                    </Button>
                  </div>
                </form>
                ...
                <div className="mt-16 pt-8 border-t border-border">
                  {mode === "forgot-password" ? <button type="button" onClick={() => setMode("signin")} className="flex items-center gap-2 text-[11px] uppercase tracking-[0.1em] text-muted-foreground hover:text-foreground transition-colors">
                      <ArrowLeft className="h-3 w-3" />
                      Back to Log In
                    </button> : <button type="button" onClick={() => setMode(mode === "signup" ? "signin" : "signup")} className="text-[11px] uppercase tracking-[0.1em] text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="hidden lg:block w-1/2 relative bg-muted">
          <img src="https://images.unsplash.com/photo-1506126613408-eca07ce68773?q=80&w=1999&auto=format&fit=crop" alt="" className="absolute inset-0 w-full h-full object-cover grayscale" />
          <div className="absolute inset-0 bg-background/10" />
        </div>
      </section>

      {/* Module Sections */}
      {modules.map((module, index) => <ModuleSection key={index} {...module} />)}
    </div>;
}