...
const generateInitialContent = (questions: {
  text: string;
}[]) => JSON.stringify({
  type: "doc",
...
const extractAnswersFromContent = (contentJSON: string, questions: {
  id: string;
  text: string;
}[]): {
  question_id: string;
  ...
  try {
    ...
    for (const node of parsed.content) {
      if (node.type === "heading" && node.attrs?.level === 2) {
        if (currentQuestionId) answers.push({
...
export default function Journal() {
  ...
  const currentSkin = useMemo(() => JOURNAL_SKINS.find(s => s.id === currentSkinId) || JOURNAL_SKINS[0], [currentSkinId]);
  useEffect(() => {
    if (!user) return;
    supabase.from("journal_entries").select("*").eq("user_id", user.id).order("entry_date", {
      ...
      setEntries((data || []).map(e => ({
        ...
        contentJSON: typeof e.text_formatting === "string" ? e.text_formatting : JSON.stringify(e.text_formatting) || "",
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    supabase.from("journal_entries").select("*").eq("user_id", user.id).eq("entry_date", dateStr).maybeSingle().then(async ({
      data: entryData
    }) => {
      if (entryData) {
        const {
          data: answersData
        } = await supabase.from("journal_answers").select("*").eq("journal_entry_id", entryData.id);
        const contentJSON = typeof entryData.text_formatting === "string" ? entryData.text_formatting : JSON.stringify(entryData.text_formatting) || "";
        setCurrentEntry({
          id: entryData.id,
        ...
        setContent(answersData?.length ? JSON.stringify({
          type: "doc",
        ...
        setCurrentAnswers([]);
        setContent(template.applyOnNewEntry ? generateInitialContent(template.questions) : JSON.stringify({
          type: "doc",
          content: [{
            type: "paragraph"
          }]
    ...
    try {
      ...
      if (currentEntry) {
        ...
        for (const answer of extractedAnswers) {
          ...
          existing ? await supabase.from("journal_answers").update({
            answer_text: answer.answer_text,
            updated_at: new Date().toISOString()
          }).eq("id", existing.id) : await supabase.from("journal_answers").insert({
      ...
      if (currentEntry) {
        const {
          data
        } = await supabase.from("journal_answers").select("*").eq("journal_entry_id", currentEntry.id);
  ...
  return <div className={`flex flex-col w-full flex-1 ${isFullscreen ? "fixed inset-0 z-50" : "-mx-6 -mt-6"}`} style={{
    ...
    minHeight: "100vh"
      ...
      <div className="sticky top-0 z-40 border-b px-4 py-2 flex items-center justify-between gap-3 flex-wrap" style={{
      backgroundColor: currentSkin.pageBg,
      ...
      <div className={`grid gap-6 w-full px-4 pt-4 pb-8 ${isFullscreen ? "grid-cols-1" : "grid-cols-1 lg:grid-cols-[1fr_320px]"}`}>
        <div className="flex flex-col min-w-0">
          {isLoading ? <div className="flex-1 rounded-lg border flex items-center justify-center min-h-[400px]" style={{
          ...
          borderColor: currentSkin.border
            ...
            </div> : <JournalTiptapEditor ref={editorRef} content={content} onChange={handleContentChange} skinStyles={{
          editorPaperBg: currentSkin.editorPaperBg,
          text: currentSkin.text,
          mutedText: currentSkin.mutedText
        }} className="opacity-100 shadow-xl mx-[10px]" />}
        </div>

        {!isFullscreen && <div>
            <JournalSidebarPanel selectedDate={selectedDate} onDateSelect={setSelectedDate} entries={entries} onInsertPrompt={handleInsertPrompt} skin={currentSkin} />
          </div>}
      </div>

      <JournalSettingsModal open={settingsOpen} onOpenChange={setSettingsOpen} template={template} onTemplateChange={t => {
      setTemplate(t);
      localStorage.setItem("journal_template", JSON.stringify(t));
    }} currentSkinId={currentSkinId} onSkinChange={id => {
      setCurrentSkinId(id);
