...
import { QuadrantTask, computeTaskStatus, Status } from "./types";
...
export function AllTasksList({ tasks, onTaskClick, onStartTask, onCompleteTask }: AllTasksListProps) {
  ...
  const getUrgencyBadge = (urgency: string) => {
    return urgency === 'high' 
      ? <Badge variant="outline" className="text-[10px] px-1.5 py-0 bg-destructive/10 text-destructive border-destructive/30">Urgent</Badge>
      : <Badge variant="outline" className="text-[10px] px-1.5 py-0">Not Urgent</Badge>;
  };

  const getImportanceBadge = (importance: string) => {
    return importance === 'high'
      ? <Badge variant="outline" className="text-[10px] px-1.5 py-0 bg-primary/10 text-primary border-primary/30">Important</Badge>
      : <Badge variant="outline" className="text-[10px] px-1.5 py-0">Not Important</Badge>;
  };
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as FilterTab)} className="flex-1 flex flex-col min-h-0">
        <div className="px-3 pt-3">
          <TabsList className="w-full grid grid-cols-5 h-8 bg-muted/30">
            <TabsTrigger value="all" className="text-xs px-1">All ({counts.all})</TabsTrigger>
            <TabsTrigger value="upcoming" className="text-xs px-1">Up ({counts.upcoming})</TabsTrigger>
            <TabsTrigger value="ongoing" className="text-xs px-1">On ({counts.ongoing})</TabsTrigger>
            <TabsTrigger value="completed" className="text-xs px-1">Done ({counts.completed})</TabsTrigger>
            <TabsTrigger value="overdue" className="text-xs px-1 text-destructive">Due ({counts.overdue})</TabsTrigger>
          </TabsList>
        </div>
