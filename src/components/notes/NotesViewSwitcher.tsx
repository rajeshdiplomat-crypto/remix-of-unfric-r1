...
export function NotesViewSwitcher({ currentView, onViewChange }: NotesViewSwitcherProps) {
  ...
  return (
    <div className="inline-flex items-center bg-background/80 rounded-full p-1 border border-border/40 shadow-sm backdrop-blur-sm">
      {views.map((view) => {
        ...
        return (
          <button
            ...
            className={cn(
              "h-8 px-4 inline-flex items-center gap-2 text-sm font-medium rounded-full transition-all duration-200",
              active
