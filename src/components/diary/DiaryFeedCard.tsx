  ...
  return (
    <Card className="overflow-hidden max-w-full bg-card border-border/40 shadow-[0_6px_18px_hsl(210_29%_8%/0.06)] hover:shadow-[0_10px_30px_hsl(210_29%_8%/0.08)] transition-all duration-200 rounded-[10px]">
      ...
      <CardContent className="px-4 pb-0 pt-2">
        {/* Title */}
        <h3 
          className="text-base max-md:text-sm font-medium text-foreground mb-2 cursor-pointer hover:underline"
        ...
        {contentPreview ? (
          <>
            ...
            >
              <p className="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap max-md:text-[13px]">
                {contentPreview}
              </p>
            </div>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button
                  ...
                  className={cn(
                    "flex-1 flex items-center justify-center gap-2 py-2.5 text-[11px] font-medium text-muted-foreground hover:bg-muted/50 rounded transition-colors",
                    userReaction && "text-primary"
                  )}
                >
                  {userReaction ? (
                    <span className="text-xs leading-none">{userReaction}</span>
                  ) : (
