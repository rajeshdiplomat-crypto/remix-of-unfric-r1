...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'submit_inquiry': {
        const { inquiry } = body;
        if (!inquiry) throw new Error("Missing inquiry data");
        const safeInquiry = { ...inquiry, user_id: userId };
        const { data, error } = await supabaseAdmin
          .from("user_inquiries")
          .insert(safeInquiry)
          .select()
      ...
      case 'fetch_timezone': {
        const { data, error } = await supabaseAdmin
          ...
          .maybeSingle();
        if (error) throw error;
        resultData = data ? data.timezone : null;
        break;
      }
      ...
      case 'submit_consent': {
        const { consentType, granted, userAgent } = body;
        if (!consentType) throw new Error("Missing consentType");
